...
class TestCSVParser(unittest.TestCase):
    ...
    def test_parse_valid_csv(self):
        ...
        # datetimeオブジェクトとして比較
        expected_dt_user1 = datetime.datetime(2023, 1, 1, 0, 0, 0, 100000, tzinfo=datetime.timezone.utc)
        ...
        expected_dt_user2 = datetime.datetime(2023, 1, 1, 0, 0, 0, 200000, tzinfo=datetime.timezone.utc)
        self.assertEqual(requests[1].request_time, expected_dt_user2)

        self.assertEqual(requests[2].user_id, "user3")
        expected_dt_user3 = datetime.datetime(2023, 1, 1, 0, 0, 0, 300000, tzinfo=datetime.timezone.utc)
    ...
    def test_parse_sample_csv(self):
        ...
        if len(requests) > 0:
            ...
            self.assertEqual(requests[0].request_time.tzinfo, datetime.timezone.utc, "Parsed datetime should be UTC")
    ...
    def test_parse_malformed_request_time_csv(self):
        ...
        self.assertTrue("型変換に失敗しました" in str(context.exception) or "time data" in str(context.exception).lower())


    def test_parse_malformed_processing_time_csv(self):
        with self.assertRaises(ValueError) as context:
            parse_csv(self.malformed_processing_time_csv_file)
        self.assertTrue("型変換に失敗しました" in str(context.exception) or "could not convert string to float" in str(context.exception).lower())
